...
class C030Class():
	...
	def RiskAnalysis(self):
		...
		self.RSRQ_Risk = (abs((self.StandardRSRQ[0] - self.RSRQ_prediction[0][0])/(self.StandardRSRQ[0])))/10
		#self.RSRQ_Risk =(((abs(self.RSRQ[0]) - 20)*5)/170) + rsrqnoise 


		if(self.BatVol[0]-self.BatVol[1]):           self.BatVol_Risk      = (abs(self.BatVol[0]-self.BatVol[1])*4 ) / max(abs(self.BatVol[0]), abs(self.BatVol[1]))  
		if(self.BatChipTemp[0]-self.BatChipTemp[1]): self.BatChipTemp_Risk = (abs(self.BatChipTemp[0]-self.BatChipTemp[1])*4 )/ max(abs(self.BatChipTemp[0]),abs(self.BatChipTemp[1])) 
		if(self.BatAmp[0]-self.BatAmp[1]):           self.BatAmp_Risk      = (abs(self.BatAmp[0]-self.BatAmp[1])*4 ) / max(abs(self.BatAmp[0]), abs(self.BatAmp[1])) 
		if(self.BatRemCap[0]-self.BatRemCap[1]):     self.BatRemCap_Risk   = (abs(self.BatRemCap[0]-self.BatRemCap[1])*4 ) / max(abs(self.BatRemCap[0]),  abs(self.BatRemCap[1]))   
		if(self.BatRemPer[0]-self.BatRemPer[1]):     self.BatRemPer_Risk   = (abs(self.BatRemPer[0]-self.BatRemPer[1])*4 ) / max(abs(self.BatRemPer[0]),  abs(self.BatRemPer[1]))   
		if(self.CoreTemp[0]-self.CoreTemp[1]):       self.CoreTemp_Risk    = (abs(self.CoreTemp[0]-self.CoreTemp[1])*3 ) / max(abs(self.CoreTemp[0]),   abs(self.CoreTemp[1])) 
		if(self.EnvTemp[0]-self.EnvTemp[1]):         self.EnvTemp_Risk     = (abs(self.EnvTemp[0]-self.EnvTemp[1])*4 ) / max(abs(self.EnvTemp[0]),    abs(self.EnvTemp[1]))   
		if(self.HeapStatus[0]-self.HeapStatus[1]):   self.HeapStatus_Risk  = (abs(self.HeapStatus[0]-self.HeapStatus[1])*4 ) / max(abs(self.HeapStatus[0]), abs(self.HeapStatus[1]))    



		if(abs(self.IMEI[0]-self.IMEI[1]) != 0):                              self.IMEI_Risk = 4.511               + float(noise)                                                                 
		if(abs(self.IMSI[0]-self.IMSI[1]) != 0):                              self.IMSI_Risk = 4.122               + float(noise)                                                                
		if(abs(self.Wakeup[0]-self.Wakeup[1]) != 1):                          self.Wakeup_Risk = 4.664             + float(noise)                                                       
		if(abs(self.WDT[0]-self.WDT[1]) != 0):                                self.WDT_Risk = 3.881                + float(noise)                                                                  	
		if(abs(self.CellIdChangeCount[0]-self.CellIdChangeCount[1]) >= 1):    self.CellIdChangeCount_Risk = 2.203  + float(noise)                       
		if(abs(self.Jam[0]-self.Jam[1]) != 0 ):                               self.Jam_Risk = 4.90                 + float(noise)                                                                     
		if(abs(self.SleepCont[0]-self.SleepCont[1]) != 0):                    self.SleepCont_Risk = 4.91           + float(noise)                                                



		movement = gpxpy.geo.haversine_distance(self.Lat[1], self.Lon[1], self.Lat[0], self.Lon[0])                           
		if(movement):
			self.lat_Risk =  ( abs(movement-DeviceSafeMovement)*4 )/max(DeviceSafeMovement,movement)
			self.lon_Risk =  self.lat_Risk



		if( self.TX[0]-self.TX[1]> 500 ):                    
			self.TX_Risk = 4.653 + float(noise) 
		else:
			#pass
			...
			self.TX_Risk = (abs((self.StandardTX[0] - self.TX_prediction[0][0])/(self.StandardTX[0])))/10



		if( self.RX[0]-self.RX[1]> 500 ):                    
			self.RX_Risk = 4.342 + float(noise) 
		else:
			#pass
			RXTimeHistory = (np.delete(np.array(self.TIME_ready_RX), [0])).reshape(len(np.array(self.TIME_ready_RX))-1, 1) 
			RXModel = SVR(kernel='linear',C=0.01,gamma='auto',epsilon=0.01).fit(RXTimeHistory , np.delete(self.RX_ready,[0])) 
			self.RX_prediction.append(RXModel.predict(self.TIME_ready_RX[0]))
			self.RX_Risk = (abs((self.RX_ready[0] - self.RX_prediction[0][0])/(self.RX_ready[0])))/10
	...
	def TotalRiskAnalysis(self):
		#noise = "%.3f" %(random.uniform(0.001, 0.099))
